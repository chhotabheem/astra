...
TEST_F(SharedQueueTest, Backpressure) {
  ...
  EXPECT_FALSE(queue.submit(
      {3, obs::Context{}, std::function<void()>([] {})})); // Rejected
...
TEST_F(SharedQueueTest, BackpressureWithLargeLimit) {
  ...
  for (int i = 0; i < 100; ++i) {
    EXPECT_TRUE(queue.submit(
        {(uint64_t)i, obs::Context{}, std::function<void()>([] {})}));
  }
...
TEST_F(SharedQueueTest, MessagePayloadIsExecuted) {
  ...
  for (int i = 0; i < 10; ++i) {
    queue.submit(
        {(uint64_t)i, obs::Context{}, std::function<void()>([&counter] {
           counter++;
         })});
  }
...
TEST_F(SharedQueueTest, ConcurrentSubmission) {
  ...
  for (int i = 0; i < 10; ++i) {
    submitters.emplace_back([&queue, &processed]() {
      for (int j = 0; j < 100; ++j) {
        queue.submit(
            {(uint64_t)j, obs::Context{}, std::function<void()>([&processed] {
               processed++;
             })});
      }
    });
  ...
  for (int i = 0; i < 100; ++i) {
    queue.submit({(uint64_t)i, obs::Context{}, std::function<void()>([&] {
                    ...
                    std::this_thread::sleep_for(
                        1ms); // Slow down to allow distribution
                  })});
  }
