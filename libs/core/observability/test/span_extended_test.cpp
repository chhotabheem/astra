...
TEST_F(SpanExtendedTest, ContextPropagation10Levels) {
    ...
    span10->end(); span9->end(); span8->end(); span7->end(); span6->end();
    span5->end(); span4->end(); span3->end(); span2->end(); span1->end();
    span0->end();
}
...
TEST_F(SpanExtendedTest, ConcurrentSpanCreation100Threads) {
    ...
    for (int i = 0; i < 100; ++i) {
        threads.emplace_back([local_tracer, i]() {
            for (int j = 0; j < 10; ++j) {
                auto span = local_tracer->start_span("thread." + std::to_string(i) + ".span." + std::to_string(j));
