...
TEST_F(LogExtendedTest, MixedLogLevelsConcurrent) {
    ...
    threads.emplace_back([]() {
        for (int i = 0; i < 100; ++i) obs::trace("trace");
    });
    threads.emplace_back([]() {
        for (int i = 0; i < 100; ++i) obs::debug("debug");
    });
    threads.emplace_back([]() {
        for (int i = 0; i < 100; ++i) obs::info("info");
    });
    threads.emplace_back([]() {
        for (int i = 0; i < 100; ++i) obs::warn("warn");
    });
    threads.emplace_back([]() {
        for (int i = 0; i < 100; ++i) obs::error("error");
    });
    threads.emplace_back([]() {
        for (int i = 0; i < 100; ++i) obs::fatal("fatal");
    });
