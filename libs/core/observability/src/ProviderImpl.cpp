...
bool ProviderImpl::init(const ::observability::Config& config) {
    ...
    try {
        ...
        auto span_processor = opentelemetry::sdk::trace::SimpleSpanProcessorFactory::Create(std::move(span_exporter));
        
        auto tracer_provider_unique = opentelemetry::sdk::trace::TracerProviderFactory::Create(
            std::move(span_processor), resource_attrs);
        m_tracer_provider = nostd::shared_ptr<trace_api::TracerProvider>(tracer_provider_unique.release());
        ...
        auto log_processor = opentelemetry::sdk::logs::SimpleLogRecordProcessorFactory::Create(std::move(log_exporter));
        
        auto logger_provider_unique = opentelemetry::sdk::logs::LoggerProviderFactory::Create(std::move(log_processor), resource_attrs);
        m_logger_provider = nostd::shared_ptr<logs_api::LoggerProvider>(logger_provider_unique.release());
...
bool ProviderImpl::shutdown() {
    ...
    // Clear instruments
    for (auto& counter : m_counters) counter.reset();
    for (auto& histogram : m_histograms) histogram.reset();
    for (auto& gauge : m_gauges) gauge.reset();
