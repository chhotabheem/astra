...
void SubmitNeverCrashes(const std::string& host, uint16_t port,
    ...
    try {
        ...
        client.submit(host, port, method, path, body, {},
            [&done](auto) { done = true; });
...
void ResponseConstructionNeverCrashes(int status_code, 
                                      const std::string& body,
...
void AllMethodsNeverCrash(const std::string& method) {
    ...
    try {
        ...
        client.submit("127.0.0.1", 19999, method, "/test", "", {},
            [&done](auto) { done = true; });
...
void PathWithSpecialCharsNeverCrashes(const std::string& path) {
    ...
    try {
        ...
        client.submit("127.0.0.1", 19999, "GET", path, "", {},
            [&done](auto) { done = true; });
