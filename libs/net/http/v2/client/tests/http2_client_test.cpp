...
TEST_F(Http2ClientTest, SubmitToMultiplePeersWorks) {
    ...
    client.submit("127.0.0.1", 19999, "GET", "/a", "", {}, [&](auto) { count++; });
    client.submit("127.0.0.1", 19998, "GET", "/b", "", {}, [&](auto) { count++; });
    client.submit("127.0.0.1", 19997, "GET", "/c", "", {}, [&](auto) { count++; });
...
TEST_F(Http2ClientTest, SubmitToSamePeerMultipleTimes) {
    ...
    while (count < 5) { std::this_thread::yield(); }
...
TEST_F(Http2ClientTest, ConcurrentSubmitsFromMultipleThreads) {
    ...
    while (count < 10) { std::this_thread::yield(); }
...
TEST_F(Http2ClientTest, SubmitToDifferentHostsSamePort) {
    ...
    client.submit("127.0.0.1", 19999, "GET", "/a", "", {}, [&](auto) { count++; });
    client.submit("127.0.0.2", 19999, "GET", "/b", "", {}, [&](auto) { count++; });
    client.submit("127.0.0.1", 19998, "GET", "/c", "", {}, [&](auto) { count++; });
...
TEST_F(Http2ClientTest, SubmitWithAllHttpMethods) {
    ...
    client.submit("127.0.0.1", 19999, "GET", "/resource", "", {}, [&](auto) { count++; });
    client.submit("127.0.0.1", 19999, "POST", "/resource", "{}", {}, [&](auto) { count++; });
    client.submit("127.0.0.1", 19999, "PUT", "/resource/1", "{}", {}, [&](auto) { count++; });
    client.submit("127.0.0.1", 19999, "DELETE", "/resource/1", "", {}, [&](auto) { count++; });
    client.submit("127.0.0.1", 19999, "PATCH", "/resource/1", "{}", {}, [&](auto) { count++; });
...
TEST_F(Http2ClientTest, HighConcurrencyMultiplePeers) {
    ...
    while (count < num_requests) { std::this_thread::yield(); }
...
TEST_F(NgHttp2ClientTest, OnCloseCallbackInvoked) {
    ...
    {
        NgHttp2Client client("127.0.0.1", 19999, m_config,
            [&close_called]() { close_called = true; },
            [&error_called](Http2ClientError) { error_called = true; });
        
        client.submit("GET", "/test", "", {}, [&done](auto) { done = true; });
...
TEST_F(ClientDispatcherTest, SubmitToMultiplePeersCreatesMultipleClients) {
    ...
    dispatcher.submit("127.0.0.1", 19991, "GET", "/a", "", {}, [&](auto) { count++; });
    dispatcher.submit("127.0.0.1", 19992, "GET", "/b", "", {}, [&](auto) { count++; });
    dispatcher.submit("127.0.0.1", 19993, "GET", "/c", "", {}, [&](auto) { count++; });
