...
void NgHttp2Client::submit(const std::string& method, const std::string& path, 
    ...
    if (current == ConnectionState::FAILED) {
        ...
        handler(astra::outcome::Result<Http2ClientResponse, Http2ClientError>::Err(Http2ClientError::ConnectionFailed));
        return;
    }
...
void NgHttp2Client::do_submit(const std::string& method, const std::string& path,
    ...
    boost::asio::post(m_io_context, [this, method, path, body, headers, handler]() {
        if (m_state.load(std::memory_order_acquire) != ConnectionState::CONNECTED) {
            ...
            handler(astra::outcome::Result<Http2ClientResponse, Http2ClientError>::Err(Http2ClientError::NotConnected));
            return;
        }
        ...
        if (ec) {
            obs::debug("do_submit: submit failed - ec=" + std::to_string(ec.value()) + " msg=" + ec.message());
            ...
            handler(astra::outcome::Result<Http2ClientResponse, Http2ClientError>::Err(Http2ClientError::SubmitFailed));
            return;
        }
        ...
        timer->async_wait([req, stream, handler](const boost::system::error_code& ec) {
            if (!ec && !stream->completed) {
                ...
                handler(astra::outcome::Result<Http2ClientResponse, Http2ClientError>::Err(Http2ClientError::RequestTimeout));
            }
        });
        ...
        req->on_close([stream, timer, handler](uint32_t error_code) {
            ...
            if (error_code != 0) {
                ...
                handler(astra::outcome::Result<Http2ClientResponse, Http2ClientError>::Err(Http2ClientError::StreamClosed));
            } else {
                handler(astra::outcome::Result<Http2ClientResponse, Http2ClientError>::Ok(
                    Http2ClientResponse(stream->status_code, std::move(stream->body), std::move(stream->headers))
                ));
            }
        });
    });
