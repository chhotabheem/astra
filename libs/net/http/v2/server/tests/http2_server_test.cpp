...
TEST(Http2ServerTest, Construction) {
    auto server = std::make_unique<http2server::Server>(make_config("127.0.0.1", 9001));
...
TEST(Http2ServerTest, ThreadConfiguration) {
    EXPECT_NO_THROW({
        auto server1 = std::make_unique<http2server::Server>(make_config("127.0.0.1", 9004, 1));
        auto server2 = std::make_unique<http2server::Server>(make_config("127.0.0.1", 9005, 2));
        auto server4 = std::make_unique<http2server::Server>(make_config("127.0.0.1", 9006, 4));
    });
}

TEST(Http2ServerTest, BindToAllInterfaces) {
    auto server = std::make_unique<http2server::Server>(make_config("0.0.0.0", 9007));
    EXPECT_NE(server, nullptr);
}

TEST(Http2ServerTest, StressConstruction) {
    for(int i=0; i<100; ++i) {
        auto server = std::make_unique<http2server::Server>(make_config("127.0.0.1", 9008));
        EXPECT_NE(server, nullptr);
    }
}
...
protected:
    void SetUp() override {
        server_ = std::make_unique<http2server::Server>(make_config("127.0.0.1", 9009));
    }
