...
MongoClient::MongoClient() : m_client(nullptr) {
  // The mongocxx::instance constructor and destructor initialize and shut down
  // the driver, respectively. Therefore, a mongocxx::instance must be created
  // before using the driver and must remain alive for as long as the driver is
  // in use.
...
void MongoClient::insertOne(const std::string &database,
...
void MongoClient::updateMany(const std::string &database,
                             const std::string &collection,
...
void MongoClient::deleteMany(const std::string &database,
                             const std::string &collection,
                             const bsoncxx::document::view &filter) {
  if (!isConnected()) {
