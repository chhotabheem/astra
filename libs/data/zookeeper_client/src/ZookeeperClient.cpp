...
public:
  ...
  Impl(const std::string &conn_str) : connection_string(conn_str) {
    ...
    if (!zh) {
      std::cerr << "Failed to connect to Zookeeper at " << conn_str
                << std::endl;
    }
#else
  ...
  bool create(const std::string &path, const std::string &value,
    ...
    return rc == ZOK;
#else
    std::cout << "[MOCK] Creating node " << path << " with value " << value
              << std::endl;
    (void)ephemeral;
    return true;
#endif
...
bool ZookeeperClient::create(const std::string &path, const std::string &value,
                             bool ephemeral) {
