...
int UriShortenerApp::run() {
    ...
    m_components.server->router().get("/health", [](std::shared_ptr<astra::router::IRequest>, 
                                          std::shared_ptr<astra::router::IResponse> res) {
        res->set_status(200);
        res->set_header("Content-Type", "application/json");
    ...
    obs::info("Using message-based architecture", {{"workers", std::to_string(m_components.pool->worker_count())}});
    obs::info("Load shedder enabled", {{"max_concurrent", std::to_string(m_components.load_shedder->max_concurrent())}});
