...
astra::outcome::Result<UriShortenerApp, BuilderError> UriShortenerBuilder::bootstrap() {
    ...
    if (load_result.is_err()) {
        ...
        return astra::outcome::Result<UriShortenerApp, BuilderError>::Err(BuilderError::InvalidConfig);
    }
...
UriShortenerBuilder& UriShortenerBuilder::useCases() {
    m_components.shorten = std::make_shared<application::ShortenLink>(m_components.repo, m_components.gen);
...
UriShortenerBuilder& UriShortenerBuilder::httpClient() {
    ...
    m_components.http_client = std::make_unique<astra::http2::Http2Client>(client_config);
    return *this;
}
...
UriShortenerBuilder& UriShortenerBuilder::pool() {
    ...
    m_components.obs_msg_handler = std::make_unique<ObservableMessageHandler>(*m_components.msg_handler);
    m_components.pool = std::make_unique<astra::execution::StickyQueue>(workers, *m_components.obs_msg_handler);
...
UriShortenerBuilder& UriShortenerBuilder::wrapObservable() {
    m_components.obs_req_handler = std::make_unique<ObservableRequestHandler>(*m_components.req_handler);
    return *this;
}
...
UriShortenerBuilder& UriShortenerBuilder::loadShedder() {
    ...
    m_components.load_shedder = std::make_unique<astra::resilience::AtomicLoadShedder>(std::move(policy));
    return *this;
}

astra::outcome::Result<UriShortenerApp, BuilderError> UriShortenerBuilder::build() {
    ...
    m_components.server = std::make_unique<astra::http2::Http2Server>(bootstrap.server());
