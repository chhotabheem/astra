...
std::string UriService::shorten(const std::string& long_url) {
    ...
    // 3. Save mapping
    repository_->save(short_code, long_url);

    return short_code;
}

std::optional<std::string> UriService::expand(const std::string& short_code) {
